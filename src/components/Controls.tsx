...
import { VideoQuality, Chapter, ControlOptions } from '../types/video';
...
const PictureInPictureIcon = memo(() => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
    <path d="M21 15V6a2 2 0 0 0-2-2H5a2 2 0 0 0-2 2v9" />
    <path d="M21 15l-3.086-3.086a2 2 0 0 0-2.828 0L6 21" />
    <rect x="12" y="15" width="10" height="7" rx="2" />
  </svg>
));

const TheaterModeIcon = memo(() => (
  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor"
    strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const CaptionsIcon = memo(({ isActive = false }: { isActive?: boolean }) => {
  if (isActive) {
...
export const Controls = memo<ControlsProps>(({
  ...
  // Visibility toggles (default to true if not specified)
  const showSeekBar = controlOptions?.showSeekBar !== false;
  const showPlayButton = controlOptions?.showPlayButton !== false;
  const showVolumeControl = controlOptions?.showVolumeControl !== false;
  const showTimeDisplay = controlOptions?.showTimeDisplay !== false;
  const showPlaybackSpeed = controlOptions?.showPlaybackSpeed !== false;
  const showQualitySelector = controlOptions?.showQualitySelector !== false;
  const showCaptionsButton = controlOptions?.showCaptionsButton !== false;
  const showFullscreenButton = controlOptions?.showFullscreenButton !== false;
  const showPictureInPictureButton = controlOptions?.showPictureInPictureButton !== false;
  const showTheaterModeButton = controlOptions?.showTheaterModeButton !== false;
  const showSocialShare = controlOptions?.showSocialShare !== false;
  const showChapterMarkers = controlOptions?.showChapterMarkers !== false;
  ...
  return (
    <div className="rvp-video-controls">
      ...
      <div className="rvp-controls-bar">
        <div className="rvp-controls-left">
          {showPlayButton && (
            ...
            >
              ...
              <span className="rvp-tooltip">{isPlaying ? 'Pause (k)' : 'Play (k)'}</span>
            </button>
          )}
        ...
        <div className="rvp-controls-right" onClick={(e) => e.stopPropagation()}>
          {showSocialShare && enableSocialShare && url && (
          ...
          {showCaptionsButton && hasCaptions && (
            <button
            ...
            >
              ...
              <span className="rvp-tooltip">Subtitles/CC (c)</span>
            </button>
          )}

          {showQualitySelector && qualities && qualities.length > 0 && onQualityChange && (
            <div onClick={(e) => e.stopPropagation()}>
              <QualitySelector
                qualities={qualities}
          ...
          {showPictureInPictureButton && enablePictureInPicture && onTogglePictureInPicture && (
            <button
              className={`rvp-control-btn ${isPictureInPicture ? 'rvp-active' : ''}`}
              onClick={onTogglePictureInPicture}
              title="Picture-in-Picture"
              aria-label={isPictureInPicture ? 'Exit Picture-in-Picture' : 'Enter Picture-in-Picture'}
            >
              <PictureInPictureIcon />
              <span className="rvp-tooltip">{isPictureInPicture ? 'Exit Picture-in-Picture' : 'Picture-in-Picture'}</span>
            </button>
          )}

          {showTheaterModeButton && enableTheaterMode && onToggleTheaterMode && (
            <button
              className={`rvp-control-btn ${isTheaterMode ? 'rvp-active' : ''}`}
              onClick={onToggleTheaterMode}
              title="Theater Mode"
              aria-label={isTheaterMode ? 'Exit Theater Mode' : 'Enter Theater Mode'}
            >
              <TheaterModeIcon />
              <span className="rvp-tooltip">{isTheaterMode ? 'Exit theater mode (t)' : 'Theater mode (t)'}</span>
            </button>
          )}
          ...
          {showFullscreenButton && (
            ...
            >
              ...
              <span className="rvp-tooltip">{isFullscreen ? 'Exit fullscreen (f)' : 'Fullscreen (f)'}</span>
            </button>
          )}
        </div>
      </div>
